...
export class AuthService {
  ...
  storeUserData(token, user){
    localStorage.setItem('id_token', token); //the data has to be a string.
  ...
  loggedIn() {
    ...
    const valu = JSON.parse(data); // the data is always a string.Parse the data with JSON.parse(), and the data becomes a JavaScript object
    console.log('the user name is ------>>>>', valu.name , '>>>>>');
    return !isExpired;
  }
