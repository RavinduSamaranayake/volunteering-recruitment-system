...
export class EventService {
  ...
  updateEvent(id, updatedevent) {
    ...
    return this.http
      .put("http://localhost:3000/events/updateEvent/" + id, updatedeventData)
      .subscribe(response => {});
  }

  deleteEvent(eventId: string) {
    return this.http
      .delete("http://localhost:3000/events/delevent/" + eventId)
      .subscribe(item => {});
  }
  getAllEvent() {
    // let headers = new HttpHeaders();
  ...
  getEventbyOrganization(orgid) {
    ...
    return this.http.get(
      "http://localhost:3000/events/selecteventbyorg/" + orgid
    );
  }
  ...
  checkUserGoing(goingevent) {
    ...
    return this.http.post(
      "http://localhost:3000/events/checkgoing",
      goingevent,
      { headers: headers }
    );
  }
  ...
  getUserEvents(volunteerID) {
    return this.http.get(
      "http://localhost:3000/events/getusereventbyid/" + volunteerID
    );
  }

  getEventVolunteers(eventID) {
    return this.http.get(
      "http://localhost:3000/events/geteventuserbyid/" + eventID
    );
  }
}
